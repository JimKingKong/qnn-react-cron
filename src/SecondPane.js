...
function SecondPane(props) {
  ...
  const aStartTobB = <InputNumber disabled={currentRadio !== 3} min={30} max={59} value={offset} size='small' onChange={onChangeOffset} style={{ width: 100 }} />

  return (
    <RadioGroup name='radiogroup' value={currentRadio} onChange={onChangeRadio}>
      <Radio style={radioStyle} value={1}>
        {everyTime.second || '每三十秒钟'}
      </Radio>
      ...
      <Radio style={radioStyle} value={3}>
        {aStartTob.second ? (
          aStartTob.second(aStartTobA, aStartTobB)
        ) : (
					<>
						从&nbsp;
  {aStartTobA}
